...
# 1. Model SVM z domyślnymi parametrami
svm_model_label = SVC(random_state=42)
svm_model_label.fit(X_train_scaled, y_train_encoded)

# Predykcje na zbiorze testowym
y_pred_label = svm_model_label.predict(X_test_scaled)

# Ocena modelu
accuracy_label = accuracy_score(y_test_encoded, y_pred_label)
precision_label = precision_score(y_test_encoded, y_pred_label)
recall_label = recall_score(y_test_encoded, y_pred_label)
f1_label = f1_score(y_test_encoded, y_pred_label)

# Zapis wyników do pliku
save_results_to_file("svm_label_results.txt", "SVM with default parameters:")
save_results_to_file("svm_label_results.txt", f"Accuracy: {accuracy_label:.2f}")
save_results_to_file("svm_label_results.txt", f"Precision: {precision_label:.2f}")
save_results_to_file("svm_label_results.txt", f"Recall: {recall_label:.2f}")
save_results_to_file("svm_label_results.txt", f"F1 Score: {f1_label:.2f}")
save_results_to_file("svm_label_results.txt", "\n" + classification_report(y_test_encoded, y_pred_label))
...
grid_search = GridSearchCV(estimator=SVC(random_state=42), param_grid=param_grid, cv=3, n_jobs=-1, verbose=2)
...
save_results_to_file("svm_label_results.txt", f"Best Parameters: {best_params}")
save_results_to_file("svm_label_results.txt", f"Accuracy: {accuracy_best:.2f}")
save_results_to_file("svm_label_results.txt", f"Precision: {precision_best:.2f}")
save_results_to_file("svm_label_results.txt", f"Recall: {recall_best:.2f}")
save_results_to_file("svm_label_results.txt", f"F1 Score: {f1_best:.2f}")
save_results_to_file("svm_label_results.txt", "\n" + classification_report(y_test_encoded, y_pred_best))

print("SVM Classifier for 'label' completed. Results are saved in svm_label_results.txt.")